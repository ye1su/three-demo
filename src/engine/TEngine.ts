...
export class ThreeEngine {
  ...
  init() {
    // 实例化相机
    this.camera = new THREE.PerspectiveCamera(70, this.dom.offsetWidth / this.dom.offsetHeight, 1, 5000)
  ...
  loadMolecule(modelUrl: string, _THIS?: any) {
    ...
    _this.loader.load(url, (pdb) => {
      ...
      for (let i = 0; i < positionBonds.count; i += 2) {
        ...
        const radians = Math.acos(_this.tmpVec3.set(0, 1, 0).dot(_this.tmpVec4.copy(_this.tmpVec1).normalize()))
